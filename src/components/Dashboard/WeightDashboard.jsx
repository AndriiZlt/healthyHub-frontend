...
const WeightDashboard = props => {
  ...
  const userYearData = useDashboardYear(); // eslint-disable-line
  ...
  for (let i = 0; i < monthNames.length; i++) {
    months.push(
      <li key={i}>
        <p className={css.weightDashboardKg}>
          {/* SERVER DATA, ЗАБРАТИ 70 */ 70}
        </p>
  ...
  if (time === 'month') {
    return (
      <div className={css.WeightDashboardContainer}>
        <ul className={css.weightDashboardList}>{days}</ul>
      </div>
    );
  }
  ...
  if (time === 'year') {
    return (
      <div className={[css.dashboardContainer, css.WeightDashboardContainer]}>
        <ul
          className={css.weightDashboardList}
          style={{
